...
fn paint_frame_buffer(system_table: &mut SystemTable<Boot>) {
    let gop = match system_table
        .boot_services()
    ...
    let pixels_line = gop.current_mode_info().stride();

    write(
        system_table.stdout(),
        format_args!(
            "Resolution: {:?}, Pixel Format: {:?}, {} pixels/line\n",
            resolution, pixel_format, pixels_line
        ),
    )
    .unwrap();
    ...
    write(
        system_table.stdout(),
        format_args!("Frame buffer size {}\n", frame_buffer_size),
    )
    .unwrap();

    for i in 0..frame_buffer_size {
        unsafe {
            frame_buffer.write_value(i, 255);
        }
    }
}
...
fn main(handle: Handle, mut system_table: SystemTable<Boot>) -> Status {
    ...
    paint_frame_buffer(&mut system_table);
